...
# For Python 3.6:
from ps2_verify_movement39.pyc import testRobotMovement
...
class RectangularRoom(object):
    ...
    def cleanTileAtPosition(self, pos):
        ...
        pos: a Position
        """
        if self.tiles[(math.floor(pos.getX()),math.floor(pos.getY()))] == False:
    ...
    def isPositionInRoom(self, pos):
        ...
        x = pos.getX()
        y = pos.getY()
        return (x >= 0 and x < float(self.width)) and (y >= 0 and y < float(self.height))
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for i in range(num_trials):
        ...
        for j in range(num_robots):
            ...
            allRobot = allRobot + [newRobot]

        while(room.getNumCleanedTiles()/room.getNumTiles()<=min_coverage):
            timeSteps += 1
            for robot in allRobot:
